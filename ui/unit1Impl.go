...
func (f *TForm1) OnFormCreate(sender vcl.IObject) {
	f.SetCaption("iFofa by lz520520")
	//不允许最大化
	...
	// 复制此列
	subMenu7 := vcl.NewMenuItem(f)
	subMenu7.SetCaption("复制此列")
	//subMenu7.SetShortCutFromString("Ctrl+C")
	subMenu7.SetOnClick(func(object vcl.IObject) {
...
func (f *TForm1) OnListView1DblClick(sender vcl.IObject) {
	...
	if f.subItemHit.IItem != -1 {
		...
		case 0:
		case 1:
			item := f.ListView1.Items().Item(f.subItemHit.IItem).SubItems()
			target := item.Strings(0)
			if !regexp.MustCompile(`^http(s)?://`).MatchString(target) {
				target = "http://" + target
			}
			win.ShellExecute(vcl.Application.MainFormHandle(), "", target, "", "", win.SW_SHOWNORMAL)
...
func (f *TForm1) DataReq(apiCh chan string) {
	//对请求语法进行base64编码
...
func (f *TForm1) DataResp(apiCh chan string) {
	respMsg := <-apiCh
	vcl.ThreadSync(func() {
		if strings.Contains(respMsg, "err=") {
			vcl.ShowMessage(respMsg)
		} else if strings.Contains(respMsg, "errmsg") {
			f.str2json3(respMsg, resultErr)
		} else {
			//数据请求成功时记录已经请求的数量
			if f.C == 1 {
				//results为一维数组时
				f.str2json1(respMsg, resultJson1)
			} else {
				//results为二维数组时
				f.str2json2(respMsg, resultJson2)
			}
		}
		f.Button3.SetEnabled(true)
	})
	close(apiCh)
}
...
func (f *TForm1) CopyColumns(sender *vcl.TListView) {
	p := f.ListView1.ScreenToClient(vcl.Mouse.CursorPos())
	f.subItemHit.Pt.X = p.X
	f.subItemHit.Pt.Y = p.Y
	win.ListView_SubItemHitTest(f.ListView1.Handle(), &f.subItemHit)
	if f.subItemHit.IItem != -1 {
		...
		if f.subItemHit.ISubItem != 0 {
			...
			for i = 0; i < sender.Items().Count(); i++ {
				buffer += sender.Items().Item(i).SubItems().Strings(f.subItemHit.ISubItem-1) + "\r\n"

			}
			vcl.Clipboard.SetTextBuf(buffer)
		}
	}

	//if f.subItemHit.IItem != -1 {
	//    index := f.subItemHit.IItem
	//    switch index {
	//    case 0:
	//    default:
	//        buffer := ""
	//        var i int32
	//        for i = 0; i < sender.Items().Count(); i++ {
	//            buffer += sender.Items().Item(i).SubItems().Strings(index-1) + "\r\n"
	//
	//        }
	//        vcl.Clipboard.SetTextBuf(buffer)
	//    }
