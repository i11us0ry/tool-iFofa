...
//::private::
type TForm5Fields struct {

}

func (f *TForm5) OnFormCreate(sender vcl.IObject){
    f.ScreenCenter()
    f.SetColor(0x241813)
...
//数量改变
func (f *TForm5) OnReqInfoChange(sender vcl.IObject) {
    val, _ := strconv.Atoi(f.Edit1.Text())
    f.ReqNum.Num = val
}

//确认选项
func (f *TForm5) OnConReqInfo(sender vcl.IObject) {
    t := reflect.TypeOf(f.CheckState)
    v := reflect.ValueOf(f.CheckState)
    //打开文件
    iniFile := vcl.NewIniFile(".\\config.ini")
...
//重置选项
func (f *TForm5) OnClearReqInfo(sender vcl.IObject) {
    //取消选项
